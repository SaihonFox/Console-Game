...
namespace Console_Game
{
    class Menu
    {
        ...
        public void Run()
        {
            ...
            {
                Console.Clear();
                if(Directory.Exists(@"Assets\Title"))
                {
                    if (Directory.GetFiles(@"Assets\Title").Length > 0)
                        foreach (string key in Directory.GetFiles(@"Assets\Title"))
                        {
                            if (key.EndsWith(".title"))
                            {
                                ShowTitle(File.ReadAllText(key));
                                break;
                            }
                        }
                    else ShowTitle();
